...
extern volatile float g_current_heading;       // <-- ADDED
extern volatile float g_current_tilt_x;        // <-- ADDED
extern volatile float g_current_tilt_y;        // <-- ADDED
...
// Timer callback: Print and send UDP telemetry
static bool print_cb(repeating_timer_t *t) {
    ...
    float heading = g_current_heading;           // <-- ADDED
    float tilt_x = g_current_tilt_x;             // <-- ADDED
    float tilt_y = g_current_tilt_y;             // <-- ADDED
    ...
    printf("USS: %-3lu cm | HDG: %.1f° | Tilt X: %.1f° Y: %.1f°\n",
           (unsigned long)uss_dist, heading, tilt_x, tilt_y);
