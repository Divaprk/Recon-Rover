...
#include "hardware/timer.h" // <-- ADD THIS
#include "imu.h"
...
// NOTE: The 100Hz sample rate is now gone, we just sample
// when the timer fires.
#define PRINT_INTERVAL_MS 500

// --- Axis mapping (can configure later) ---
#define MAP_AX 0
#define MAP_AY 1
#define MAP_AZ 2
#define SIGN_AX (+1)
...
static bool g_imu_ready = false; // <-- ADD THIS
...
// --- NEW TIMER CALLBACK ---
// This function will be called every PRINT_INTERVAL_MS
static bool imu_timer_cb(repeating_timer_t *t) {
    if (!g_imu_ready) return true; // Don't do anything if not initialized

    // --- Read sensors ---
    float ax,ay,az; 
    acc_read_mg(&ax,&ay,&az);
    
    int16_t mx,my,mz; 
    mag_read_raw(&mx,&my,&mz);

    // --- Print to USB ---
    printf("IMU: ax=%+7.1f ay=%+7.1f az=%+7.1f | mx=%+6d my=%+6d mz=%+6d\n", 
           ax,ay,az, mx,my,mz);
...
void imu_init(void){
    ...
    printf("IMU: I2C initialized on pins %d and %d.\n", I2C_SDA_PIN, I2C_SCL_PIN);

    // --- 2. Find sensors ---
    if(!acc_init()){
        printf("[ERROR] IMU: Accelerometer not found!\n"); 
        return; // Failed
    }
    printf("IMU: Accelerometer found at 0x%02X\n", g_acc);

    if(!mag_init()){
        printf("[ERROR] IMU: Magnetometer not found!\n"); 
        return; // Failed
    }
    printf("IMU: Magnetometer found at 0x%02X\n", g_mag);

    g_imu_ready = true; // Flag that we are ready to read

    // --- 3. Start the reporting timer ---
    ...
    printf("IMU: Reporting timer started (%d ms).\n", PRINT_INTERVAL_MS);
}